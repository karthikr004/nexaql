...
interface NodeEditorProps {
  nodeName: string;           // node name or '__policy_functions__'
  onClose: () => void;        // back to SQL Preview
  onSaved?: () => void;       // refresh ontology after save
}
...
export default function NodeEditor({ nodeName, onClose, onSaved }: NodeEditorProps) {
  ...
  // Policy functions list view state
  const [selectedFunction, setSelectedFunction] = useState<string | null>(null);

  const isPolicyFunctions = nodeName === '__policy_functions__';
  ...
  const updateNode = useCallback(
    (updater: (draft: NodeData) => void) => {
      if (isPolicyFunctions) return;
      updateOntology((o) => {
        const nd = o.nodes[nodeName];
        if (nd) updater(nd);
      });
    },
    [nodeName, isPolicyFunctions, updateOntology],
  );
  ...
  }, [ontology, isDirty, onSaved]);

  // ── Current node data ──────────────────────────────────────────────────────

  const node = !isPolicyFunctions && ontology ? ontology.nodes[nodeName] ?? null : null;
    ...
    return (
      <div className="flex h-full flex-col bg-[#0f1117]">
        <div className="flex shrink-0 items-center justify-between border-b border-[#252d3d] px-4 py-2">
          ...
          <button type="button" onClick={onClose} className="text-slate-500 hover:text-slate-300 text-sm">&times;</button>
        </div>
  ...
  const handleTestPolicy = async (policyIndex: number, condition: string) => {
    const key = `${nodeName}-${policyIndex}`;
  ...
  // ── Render: Policy Functions (list view) ───────────────────────────────────
    ...
    <div className="flex h-full flex-col overflow-hidden bg-[#0f1117]">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-[#252d3d] px-4 py-2">
        <div className="flex items-center gap-2 text-sm">
          {isPolicyFunctions ? (
            <>
              <span className="text-amber-400/90 font-semibold">Policy Functions</span>
              <span className="text-[10px] text-slate-600">(ontology-level)</span>
            </>
          ) : (
            <>
              <span className="text-slate-500">Editing:</span>
              <span className="font-mono text-slate-200">{nodeName}</span>
            </>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded border border-[#252d3d] px-3 py-1.5 text-[11px] text-slate-400 hover:border-slate-500 hover:text-slate-200"
          >
            &times; Close
          </button>
        </div>
      ...
      {isPolicyFunctions ? (
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center">
            <div className="mb-2 text-sm text-red-400">Node not found: {nodeName}</div>
            <button
              type="button"
              onClick={onClose}
