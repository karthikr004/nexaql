...
function registerLanguage(monaco: typeof MonacoType) {
...
function registerThemes(monaco: typeof MonacoType) {
...
// ── Calc function snippets ──────────────────────────────────────────────────

// biome-ignore-start lint/suspicious/noTemplateCurlyInString: Monaco snippet syntax uses ${n:placeholder}
const CALC_FUNCTIONS = [
  ...
  { label: 'EXTRACT', insert: 'EXTRACT(${1|DAY,MONTH,YEAR,HOUR,EPOCH|} FROM ${2:expr})', detail: 'numeric', doc: 'Extract part of a date/interval' },
  { label: 'DATE_TRUNC', insert: "DATE_TRUNC('${1|year,month,week,day|}', ${2:field})", detail: 'date', doc: 'Truncate date to unit' },
  { label: 'DATE_PART', insert: "DATE_PART('${1|day,month,year,hour|}', ${2:field})", detail: 'numeric', doc: 'Extract numeric date component' },
  { label: 'AGE', insert: 'AGE(${1:end_date}, ${2:start_date})', detail: 'interval', doc: 'Interval between two dates' },
  ...
  { label: 'CEIL', insert: 'CEIL(${1:expr})', detail: 'integer', doc: 'Round up' },
  { label: 'FLOOR', insert: 'FLOOR(${1:expr})', detail: 'integer', doc: 'Round down' },
  { label: 'ABS', insert: 'ABS(${1:expr})', detail: 'numeric', doc: 'Absolute value' },
  { label: 'GREATEST', insert: 'GREATEST(${1:expr1}, ${2:expr2})', detail: 'numeric', doc: 'Largest of arguments' },
  ...
  { label: 'NULLIF', insert: 'NULLIF(${1:expr}, ${2:0})', detail: 'any', doc: 'NULL if two values equal (prevent /0)' },
...
function createCompletionProvider(
  nodesRef: React.RefObject<NodeInfo[]>,
  monaco: typeof MonacoType,
): MonacoType.languages.CompletionItemProvider {
  const K = monaco.languages.CompletionItemKind;
  const ISR = monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet;

  return {
    triggerCharacters: ['{', '(', ' ', '\n', '@', ':', '_'],
    provideCompletionItems(model, position) {
      const word = model.getWordUntilPosition(position);
      const charBeforeWord = word.startColumn > 1
        ? model.getValueInRange({
            startLineNumber: position.lineNumber, endLineNumber: position.lineNumber,
            startColumn: word.startColumn - 1, endColumn: word.startColumn,
          })
        : '';
      ...
      const add = (label: string, insert: string, kind: MonacoType.languages.CompletionItemKind, detail?: string, doc?: string, sort = '5') => {
        suggestions.push({ label, kind, detail, documentation: doc, insertText: insert, insertTextRules: ISR, sortText: sort + label, range });
      };

      // Directives
      if (charBeforeWord === '@' || textBefore.match(/@\w*$/)) {
        add('@limit', '@limit(${1:10})', K.Keyword, undefined, 'Limit result rows', '1');
        add('@offset', '@offset(${1:0})', K.Keyword, undefined, 'Skip N rows', '1');
        add('@orderby', '@orderby(${1:field}, ${2|ASC,DESC|})', K.Keyword, undefined, 'Sort results', '1');
        add('@distinct', '@distinct', K.Keyword, undefined, 'Remove duplicates', '1');
        return { suggestions };
      }
      ...
      if (isInsideCalcExpr(textBefore)) {
        for (const fn of CALC_FUNCTIONS) {
          add(fn.label, fn.insert, K.Function, fn.detail, fn.doc, '1');
        }
        const stack = getNodeStack(textBefore);
        const topName = stack[stack.length - 1] ?? '';
        const node = nodes.find((n) => n.name === topName);
        if (node) {
          for (const f of node.fields) { add(f.name, f.name, K.Field, f.type, undefined, '2'); }
        }
        return { suggestions };
      }
      ...
      // Inside filter parens
      let filterNode = currentNode;
      if (!filterNode && isInsideFilterParens(textBefore)) {
        const parenMatch = textBefore.match(/(\w+)\s*\([^)]*$/);
        if (parenMatch) { filterNode = nodes.find((n) => n.name === parenMatch[1]); }
      }
      if (isInsideFilterParens(textBefore) && filterNode) {
        const fieldName = getFieldBeforeColon(textBefore);
        if (fieldName && textBefore.match(/:\s*\{\s*\w*$/)) {
          for (const [op, doc] of [
            ['eq', 'Equal to'], ['ne', 'Not equal'], ['gt', 'Greater than'],
            ['gte', 'Greater than or equal'], ['lt', 'Less than'], ['lte', 'Less than or equal'],
            ['like', 'ILIKE pattern match'], ['in', 'In list [a, b, c]'],
            ['not_in', 'Not in list'], ['null', 'IS NULL / IS NOT NULL'],
          ] as [string, string][]) {
            add(op, `${op}: \${1:value}`, K.Operator, 'operator', doc, '1');
          }
          return { suggestions };
        }
        for (const f of filterNode.fields) { add(f.name, `${f.name}: \${1:value}`, K.Field, f.type, undefined, '2'); }
        for (const sf of filterNode.specialFilters) { add(sf.name, `${sf.name}: true`, K.Value, 'special filter', sf.description, '2'); }
        add('calc(...): filter', 'calc(${1:expr}): { ${2|gt,gte,lt,lte,eq,ne|}: ${3:value} }', K.Keyword, 'calc filter', 'Filter by computed expression', '3');
        return { suggestions };
      }
      ...
      if (!currentNode) {
        ...
        if (!textBefore.trim() || textBefore.match(/^\s*(?:#[^\n]*)?\s*$/)) {
          add('query', 'query ${1:QueryName} {\n  $0\n}', K.Keyword, undefined, 'Start a new NexaQL query', '1');
        }
        for (const n of nodes) { add(n.name, `${n.name} {\n  \${1}\n}`, K.Class, n.description, undefined, '2'); }
      } else {
        // Inside node body
        add('calc(...)', '${1:alias}: calc(${2:expr})', K.Keyword, 'computed field', 'Inline computed expression', '1');
        for (const f of currentNode.fields) { add(f.name, f.name, K.Field, f.type, undefined, '2'); }
        for (const [fn, doc] of [
          ['sum', 'Sum'], ['avg', 'Average'], ['min', 'Minimum'], ['max', 'Maximum'], ['count', 'Count'],
        ] as [string, string][]) {
          add(fn, `\${1:alias}: ${fn}(\${2:field})`, K.Function, `${fn.toUpperCase()}`, doc, '3');
        }
        for (const e of currentNode.edges) {
          const target = nodes.find((n) => n.name === e.target);
          const fields = target?.fields.slice(0, 3).map((f) => `    ${f.name}`).join('\n') ?? '    ';
          add(e.name, `${e.name} {\n${fields}\n  }`, K.Module, `-> ${e.target}`, e.description, '4');
        }
        add('@limit', '@limit(${1:10})', K.Keyword, undefined, 'Limit rows', '5');
        add('@offset', '@offset(${1:0})', K.Keyword, undefined, 'Skip rows', '5');
        add('@orderby', '@orderby(${1:field}, ${2|ASC,DESC|})', K.Keyword, undefined, 'Sort', '5');
        add('@distinct', '@distinct', K.Keyword, undefined, 'Deduplicate', '5');
      }
...
export default function V2QueryEditor({
  value, onChange, onRun, insertText, onInsertConsumed, ontologyNodes = [], theme,
}: Props) {
  const editorRef = useRef<MonacoType.editor.IStandaloneCodeEditor | null>(null);
  const onRunRef = useRef(onRun);
  const onChangeRef = useRef(onChange);
  const nodesRef = useRef(ontologyNodes);
  const completionRef = useRef<MonacoType.IDisposable | null>(null);

  useEffect(() => { onRunRef.current = onRun; }, [onRun]);
  useEffect(() => { onChangeRef.current = onChange; }, [onChange]);
  useEffect(() => { nodesRef.current = ontologyNodes; }, [ontologyNodes]);
  ...
  const handleMount = useCallback(
    (editor: MonacoType.editor.IStandaloneCodeEditor, m: typeof MonacoType) => {
      ...
      completionRef.current = m.languages.registerCompletionItemProvider(LANG, createCompletionProvider(nodesRef, m));
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    if (selection) {
      editor.executeEdits('schema-explorer', [{ range: selection, text: insertText, forceMoveMarkers: true }]);
    }
    editor.focus();
    onInsertConsumed();
