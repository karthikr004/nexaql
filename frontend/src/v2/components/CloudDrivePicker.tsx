...
export default function CloudDrivePicker({ onImported, onToast }: CloudDrivePickerProps) {
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      ...
      if (provRes.ok) {
        ...
        setHasProvider((data.providers ?? []).length > 0);
      }
    } catch {
      // ignore
    } finally {
  ...
  const handleConnect = useCallback(async () => {
    setConnecting(true);
    try {
      const res = await fetch('/api/cloud-drive/connect/google_drive');
      if (!res.ok) {
        const body = await res.json();
  ...
  if (!hasProvider) {
    return (
      <div style={{ textAlign: 'center', padding: '24px 16px' }}>
        ...
        <p className="v2-body-sm" style={{ marginBottom: 4 }}>No cloud providers configured</p>
        <p style={{ fontSize: 11, color: 'var(--v2-text-tertiary)' }}>
          Add Google Drive credentials in Settings first
        </p>
      </div>
    );
  }
  ...
  if (accounts.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '24px 16px' }}>
        ...
        <p className="v2-body-sm" style={{ marginBottom: 10 }}>Connect your Google Drive to browse files</p>
        <button
          type="button"
