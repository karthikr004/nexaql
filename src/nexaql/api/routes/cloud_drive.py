...
"""Cloud drive routes — config, OAuth, file browsing, and import."""
...
@router.get("/cloud-drive/providers")
async def list_providers() -> JSONResponse:
    rows = bs.list_cloud_drive_configs()
    result = []
    for row in rows:
        result.append({
            "provider": row["provider"],
            "client_id": _mask_secret(row["client_id"]),
            "enabled": bool(row.get("enabled", True)),
        })
    return JSONResponse({"providers": result})


@router.post("/cloud-drive/providers")
async def save_provider(body: ProviderConfigBody) -> JSONResponse:
    bs.save_cloud_drive_config(
        provider=body.provider,
        client_id=body.client_id,
        client_secret=body.client_secret,
    )
    return JSONResponse({"status": "saved", "provider": body.provider})


@router.delete("/cloud-drive/providers/{provider}")
async def delete_provider(provider: str) -> JSONResponse:
    deleted = bs.delete_cloud_drive_config(provider)
    if not deleted:
...
@router.get("/cloud-drive/connect/{provider}")
async def connect(provider: str, request: Request) -> JSONResponse:
    config_row = bs.get_cloud_drive_config(provider)
    if not config_row:
        raise HTTPException(
            status_code=404,
            detail=f"Cloud drive provider '{provider}' not configured",
        )

    config = CloudDriveClientConfig(**config_row)
    drv = get_cloud_drive_provider(config)
...
def _get_account_and_manager(account_id: int) -> tuple[CloudDriveAccount, TokenManager]:
    row = bs.get_cloud_drive_account(account_id)
    if not row:
        raise HTTPException(status_code=404, detail="Account not found")
    account = CloudDriveAccount(**row)
    return account, TokenManager(account)
...
async def browse(
    ...
    account, mgr = _get_account_and_manager(account_id)
    token = await mgr.get_valid_token()

    config_row = bs.get_cloud_drive_config(account.provider)
    if not config_row:
        raise HTTPException(status_code=404, detail="Provider not configured")
